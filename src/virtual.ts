...
//   DYNAMIC: 'DYNAMIC',
// };
import { DIRECTION_TYPE, Range, CALC_TYPE } from './types';

type NOOP = ((result: any) => void) | null;
...
export default class Virtual {
  ...
  sizes: Record<string | number, any> = {};
  firstRangeTotalSize = 0;
  firstRangeAverageSize = 0;
  lastCalcIndex = 0;
  fixedSizeValue = 0;
  calcType: keyof typeof CALC_TYPE | '' = '';
  offset = 0;
  constructor(param: any, callUpdate: NOOP) {
    this.init(param, callUpdate);
  }

  init(param: any, callUpdate: NOOP): void {
    // param data
    this.param = param;
  ...
  updateParam(key: string, value: number | any[]): void {
    if (this.param && key in this.param) {
      // if uniqueIds change, find out deleted id and remove from size map
      if (key === 'uniqueIds') {
        this.sizes.forEach((v: any, key: any) => {
  ...
  saveSize(id: string, size: number): void {
    ...
    if (this.calcType === CALC_TYPE.INIT) {
      ...
      // it's no use at all
      delete (this as any).fixedSizeValue;
    }
    ...
    if (
      ...
      if (
        ...
        // it's done using
        delete (this as any).firstRangeTotalSize;
      }
    }
  }
  ...
  getEndByStart(start: number): number {
    const theoryEnd: number = start + this.param.keeps - 1;
    const truelyEnd: number = Math.min(theoryEnd, this.getLastIndex());
    return truelyEnd;
  }
